...
namespace SnowSled
{
    inline constexpr glm::vec3 boxLeftCorner(1.5f, 0.04f, 2.0f);
    inline constexpr glm::vec3 boxRightCorner(3.6f, 0.2f, 4.1f);
    inline constexpr glm::vec3 cameraPosition(-0.4f, 2.2f, 3.0f);

    inline constexpr float zMargin = 0.5f;
    inline constexpr float sledSpeed = 3.0f;
    inline constexpr float sledCenterX = 2.1f;
    inline constexpr float sledCenterY = 0.04f;
    ...
    {
        return glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(sledCenterX, sledCenterY, sledCenterZ)), sledScale);
    }

    inline void SetLogoUniforms(const Shader &shader)
    {
        const glm::mat4 sledMatrix = glm::scale(glm::translate(glm::mat4(1.0f), glm::vec3(sledCenterX, sledCenterY, sledCenterZ)), sledScale);
