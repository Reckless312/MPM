...
VideoRecorder::VideoRecorder(const int width, const int height, const char* outputPath)
{
    ...
    char cmd[512];
    snprintf(cmd, sizeof(cmd),"ffmpeg -y -f rawvideo -pixel_format rgba -video_size %dx%d -framerate %d -i pipe:0 -c:v libx264 -pix_fmt yuv420p -crf 18 -vf vflip %s",width, height, Program::recordingFrameRate, outputPath);
...
void VideoRecorder::EndFrame()
{
    ...
    {
        fwrite(data, 1, this->width * this->height * 4, this->ffmpegPipe);
