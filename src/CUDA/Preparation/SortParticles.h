...
void SortParticles(const ParticleBlock* inputBlocks, ParticleBlock* outputBlocks, const HashTable& blockCodeToIndex, int particleCount, uint64_t* sortKeys, uint64_t* sortKeysOut, uint32_t* indices, uint32_t* sortedIndices, void* tempStorage, size_t& tempStorageBytes, cudaStream_t stream);
void WarpSort(ParticleBlock* particleBlocks, int particleCount, const HashTable& blockCodeToIndex, uint64_t* particleHomeBlockCodes, cudaStream_t stream);
